...
def reset_db():
    """Completely reset Nexa DB and recreate all tables cleanly."""
...
def call_openrouter(messages, model=MODEL, timeout=18):
    if not OPENROUTER_API_KEY:
        return "[Offline mode] No API key set."
...
def local_css():
    ...
    .chat-box { background: rgba(255,255,255,0.05); padding:12px; border-radius:12px; max-height:65vh; overflow:auto; }
...
with right:
    ...
    st.markdown("- First message becomes the chat title automatically.")
    st.markdown("- Click **Reset Database** to clear all chats.")
