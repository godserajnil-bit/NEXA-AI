...
# Final combined single-file Flask app — includes all requested features and fixes.
# Usage:
...
DB_FILE = "nexa_streamlit.db"  # SQLite database file
DB_FILE = "nexa_final.db"
UPLOAD_FOLDER = "uploads"
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

# API key placeholders (kept blank if you don't want to call external APIs)
OPENROUTER_API_KEY = "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"   # <- add your openrouter / openai key here for LLM integration
...
/* startup splash */
#splash{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:linear-gradient(180deg, rgba(0,0,0,0.75), rgba(0,0,0,0.85));z-index:9999}
#splash .logo{width:150px;height:150px;background:url('https://i.imgur.com/ObRj9jD.png') center/contain no-repeat;animation:pop 1100ms cubic-bezier(.2,.9,.2,1)}
@keyframes pop{0%{transform:scale(0.6);opacity:0}60%{transform:scale(1.05);opacity:1}100%{transform:scale(1)}}
...
<body>
<!-- Splash logo overlay -->
<div id="splash"><div class="logo" aria-hidden="true"></div></div>
...
/* Splash handling (logo) */
window.addEventListener('load', () => {
  // show splash for 900ms then reveal app
  setTimeout(()=> {
    const splash = document.getElementById('splash');
    splash.style.transition = 'opacity 400ms ease';
    splash.style.opacity = '0';
    setTimeout(()=>{ splash.remove(); document.getElementById('appRoot').style.visibility='visible'; }, 420);
  }, 700);
